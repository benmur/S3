...
import DynamoDBBucketClient from './dynamodbbucket/backend';
...
if (config.backends.metadata === 'mem') {
    ...
    implName = 'bucketfile';
} else if (config.backends.metadata === 'dynamodb') {
    client = new DynamoDBBucketClient();
    implName = 'dynamodbbucket';
